...
`awsvpnctl setup`은 Hammerspoon 메뉴바를 선택적으로 연결합니다.
...
`setup`이 메뉴바 symlink와 `init.lua` 항목을 추가합니다.
...
설치 직후 Hammerspoon CLI IPC 또는 AppleScript로 reload 신호를 보냅니다. 둘 다 비활성화되어 있으면 사용자가 직접 Hammerspoon → `Reload Config`를 실행해야 합니다.
...
`hammerspoon menubar` 항목이 `→ /.../hammerspoon/awsvpnctl_hammerspoon.lua; ctl=/.../awsvpnctl`로 표시되면 연결이 성공한 상태입니다.

## Status Source

메뉴바는 `~/.hammerspoon/awsvpnctl_config.lua`에 기록된 `awsvpnctl`로 `status --json`을 polling합니다. `setup`은 현재 checkout, shell `PATH`, Homebrew 후보 중 profile 상태를 실제로 반환하는 CLI를 config에 기록합니다. AWS config나 profile 파일 존재 여부가 아니라 실제 OpenVPN process와 `utun` 상태를 봅니다.
