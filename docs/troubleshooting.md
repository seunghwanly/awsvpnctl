...
AWS가 SAML patch와 맞지 않는 방식으로 protocol을 바꾸면 upstream patch 갱신이 필요할 수 있습니다.

## Browser Login Hangs

브라우저가 SSO 완료 후 아래 주소로 POST해야 합니다.

```text
http://127.0.0.1:35001
```

방화벽, Little Snitch, 보안 도구가 localhost callback을 막는지 확인합니다.
...
`.ovpn`에 `auth-federate`가 있는지 확인합니다.
...
프로젝트가 `~/Documents`, `~/Desktop`, `~/Downloads` 아래 있으면 macOS privacy policy 때문에 background access가 막힐 수 있습니다. `~/dev/awsvpnctl` 같은 경로를 권장합니다.

## Status Says Connected But VPN Is Down

최신 버전은 process 존재만으로 connected라고 보지 않고 `utun` 상태까지 확인합니다.
...
이전 버전에서 root-owned log가 생긴 경우가 있습니다.

```bash
awsvpnctl logs dev
```

최신 버전은 sudo runner로 log ownership을 복구하거나 fallback read를 시도합니다.

## Hammerspoon Menu Not Updated

Hammerspoon에서 `Reload Config`를 실행하거나 앱을 재시작합니다.
