...
`awsvpnctl setup`은 필요한 OpenVPN, 제한된 sudoers, LaunchAgent를 설치한 뒤 `~/Downloads`와 `~/Desktop`에서 AWS Client VPN export로 보이는 `.ovpn` 파일을 찾아 `etc/profiles/`로 가져오고, 자동 연결할 프로필을 설정합니다.
